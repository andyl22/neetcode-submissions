class Solution:
    def reorderList(self, head: Optional[ListNode]) -> None:
        ...
        prev = None
        while second_half:
            temp = second_half.next     # Save the original next node
            second_half.next = prev     # Flip the pointer backward
            prev = second_half          # Move prev forward
            second_half = temp          # Move curr forward
        ...
        #will always be null since hte prev while loop will exit when None
        first = head
        second = prev 
